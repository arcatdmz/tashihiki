...
3. **結果を確認**
   - 「けっか」タブで成績を確認
   - 成績は自動でlocalStorageに保存されます
   - URLを保存・共有すれば結果を共有できます
   - 「🔄 リセット」ボタンで成績をリセット

## 🔗 URL履歴保存機能

結果の履歴はブラウザのlocalStorageに保存されます。別の端末で共有したい場合は、結果画面で生成されるURLを共有できます。

URLで共有される内容は、正解数・不正解数・解答時間のクエリパラメータです：

```
例: ?correct=5&wrong=2&totalTime=4.259
```

このURLをブックマークやシェアすることで、結果を共有できます。
